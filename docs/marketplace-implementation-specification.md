...
- **AD-019 (Marketplace Product Model Contract، معتمد بعد Final Execution Sprint):** أي تطبيق Marketplace جديد يُمثَّل حصرًا بصف `marketplace_items` + `application_details` (key/name/category/partner/billing_model/pricing_model/entry_route) — بلا أي حقل أو شرط خاص باسم تطبيق معيَّن داخل منطق Marketplace العام (`MarketplaceController`/`MyAppsController`/`UserAppsResolver`/`DatabaseMarketplaceRepository`). حالة "مُطلَق فعليًا مقابل Coming Soon" تُحسَم حصرًا بوجود `entry_route`، لا بأي علم/Hardcoding آخر. Authorization الخاص بتطبيق مُنفَّذ (Policy/Service خاصة به فقط)، Entitlement يمر عبر `EntitlementResolver`/`EnsureMarketplaceEntitlement` الموجودين حصرًا — لا فحص مواز (AD-013). إفلاس تك أول تطبيق يُثبت هذا العقد عمليًا (`bankruptcy-tech`) — راجع `docs/marketplace-final-architecture.md`، `docs/applications/eflas-tech.md`.

**كل قسم أدناه يعكس هذي القرارات التسعة عشر كأمر واقع — لا "توصية معلَّقة" بعد الآن.**
